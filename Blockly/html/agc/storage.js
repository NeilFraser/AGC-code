...
/**
 * Hook to get the code from the editor.
 * @returns {string} XML as text.
 */
BlocklyStorage.getCode = function() {
  return Code.getXml(false);
};

/**
 * Hook to set the code in the editor.
 * @param {string} code XML as text.
 */
BlocklyStorage.setCode = function(code) {
  Code.setXml(code);
};
...
BlocklyStorage.link = function() {
  ...
  BlocklyStorage.makeRequest_('/scripts/blockly-agc/storage.py', 'POST',
...
BlocklyStorage.retrieveXml = function(key) {
  BlocklyStorage.makeRequest_(`/software/blockly-agc/data/${encodeURIComponent(key)}.blockly`,
